...
final class AIAssembly: Assembly {
    func assemble(container: Container) {
        container.register(AICredentialProvider.self) { _ in
            KeychainAICredentialProvider(keychain: BaseKeychain(
        ...
        container.register(AIContextRedactor.self) { r in
            ...
            return DefaultAIContextRedactor(knownSecrets: {
                let secretResult: Result<String?, KeychainError> = keychain
                    .getValue(String.self, forKey: NightscoutConfig.Config.secretKey)
        ...
        container.register(AITrioDataReading.self) { r in
            ...
            return TrioAIDataReader(makeContext: { CoreDataStack.shared.newTaskContext() }, readSettings: {
                ...
                if let ratios = storage.retrieve(OpenAPS.Settings.carbRatios, as: CarbRatios.self) {
                    schedules += ratios.schedule.prefix(96)
                        .map { .init(kind: "carbRatio", start: $0.start, value: $0.ratio, unit: "\(ratios.units.rawValue)/U") }
                }
                if let isf = storage.retrieve(OpenAPS.Settings.insulinSensitivities, as: InsulinSensitivities.self) {
                    schedules += isf.sensitivities.prefix(96)
                        .map { .init(kind: "ISF", start: $0.start, value: $0.sensitivity, unit: "\(isf.units.rawValue)/U") }
                }
                if let targets = storage.retrieve(OpenAPS.Settings.bgTargets, as: BGTargets.self) {
                    schedules += targets.targets.prefix(96)
                        .map {
                            .init(
                                kind: "target",
                                start: $0.start,
                                value: $0.low,
                                upperValue: $0.high,
                                unit: targets.units.rawValue
                            ) }
                }
                return AISettingsSnapshot(
                    settings: manager.settings,
                    preferences: manager.preferences,
                    pump: manager.pumpSettings,
                    schedules: schedules
                )
            })
        }
        ...
        container.register(OpenAIClient.self) { r in
            URLSessionOpenAIClient(
                credentials: r.resolve(AICredentialProvider.self)!,
                redactor: r.resolve(AIContextRedactor.self)!
            )
        }
        container.register(AIService.self) { r in
            ...
            MainActor.assumeIsolated {
                DefaultAIService(
                    store: r.resolve(AIConversationStore.self)!,
                    builder: r.resolve(AIContextBuilder.self)!,
                    redactor: r.resolve(AIContextRedactor.self)!,
                    client: r.resolve(OpenAIClient.self)!
                )
            }
        }.inObjectScope(.container)
    }
