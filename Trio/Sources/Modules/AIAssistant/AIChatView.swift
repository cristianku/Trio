        ...
        VStack(spacing: 0) {
            ScrollViewReader { proxy in
                ScrollView {
                    LazyVStack(alignment: .leading, spacing: 16) {
                        ...
                        ForEach(state.conversation(conversationID)?.messages ?? []) { message in
                            VStack(alignment: .leading, spacing: 5) {
                                HStack {
                                    Text(message.role == .user ? "You" : "AI Assistant").font(.headline)
                                    Spacer()
                ...
                .onChange(of: state.conversation(conversationID)?.messages.count) { withAnimation { proxy.scrollTo("newest", anchor: .bottom) } }
            ...
            VStack(alignment: .leading, spacing: 8) {
                if !state.configuration.enabled || !state.configuration.hasConsent || !state.hasKey {
                    NavigationLink("Configure AI and review data sharing") { AISettingsView(state: state) }
                }
                TextField("Ask about your Trio data", text: Binding(get: { state.draft(for: conversationID) }, set: { state.setDraft($0, for: conversationID) }), axis: .vertical)
                    .lineLimit(2...6)
                ...
                HStack {
                    ...
                    if state.isLoading {
                        ...
                        Button("Send", systemImage: "arrow.up.circle.fill") { state.send(conversationID: conversationID) }
                            .disabled(!state.isReady || !state.configuration.enabled || !state.configuration.hasConsent || !state.hasKey ||
                                state.draft(for: conversationID).trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || state.draft(for: conversationID).count > AIContextLimits.messageCharacters)
                    }
                }
            }.padding().background(Color.chart)
        }
        .background(appState.trioBackgroundColor(for: colorScheme))
        .navigationTitle(state.conversation(conversationID)?.title ?? "AI Chat")
