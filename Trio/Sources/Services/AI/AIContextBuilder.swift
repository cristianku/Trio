...
extension AIContextBuilder {
    func build(configuration _: AIConfiguration, interval _: DateInterval) async throws -> TrioAIContext {
...
final class TrioAIContextBuilder: AIContextBuilder {
    ...
    func build(configuration: AIConfiguration) async throws -> TrioAIContext {
        ...
        let interval = DateInterval(
            start: date.addingTimeInterval(-Double(configuration.historyHours.rawValue) * 3600),
            end: date
        )
    ...
    func build(configuration: AIConfiguration, interval: DateInterval) async throws -> TrioAIContext {
        ...
        context.notes = [
            "Settings are current, not historical. Glucose uses mg/dL. Insulin uses U and U/hour; carbs use grams.",
        ...
        if summarize {
            context.hourlySummaries = try await historySummaries(
            ...
            context.notes
                .append(
                    "hourlySummaries describe stored observations, not complete coverage or measured insulin delivery. Missing hours are unavailable, not zero events. Glucose means are sample means, not time-weighted averages or time in range. Pump amounts sum recorded bolus amounts only; never add basal rate times duration. FPU carbs are separate from recorded meals. Determination counts are recommendations, not delivered doses. Ask for a narrower period for individual records and reasons."
                )
        }
        ...
        for category in configuration.categories.sorted(by: { $0.rawValue < $1.rawValue }) {
            ...
            case .glucose:
                ...
                context.glucose = bounded(
                    try await source.glucose(in: interval, limit: limits.glucose + 1),
                    limit: limits.glucose,
                    category: category.rawValue
                )
            case .pumpHistory:
                if !summarize {
                    context.pumpHistory = bounded(
                        try await source.pumpHistory(in: interval, limit: limits.pumpHistory + 1),
                        limit: limits.pumpHistory,
                        category: category.rawValue
                    )
                }
            ...
            case .adjustments:
                ...
                if relevant.count > limits
                    .adjustments { context.notes.append("adjustments truncated to newest \(limits.adjustments) entries.") }
                context.notes
                    .append(
                        "Adjustments include recorded runs and currently enabled configurations overlapping the interval, including starts before it. Planned end dates are not observed end times. Parameters come from linked stored configurations and are unavailable if that link no longer exists."
                    )
            case .carbs:
                if summarize { continue }
                context.carbs = bounded(
                    try await source.carbs(in: interval, limit: limits.carbs + 1),
                    limit: limits.carbs,
                    category: category.rawValue
                )
            case .determinations:
                if summarize { continue }
                context.determinations = bounded(
                    try await source.determinations(in: interval, limit: limits.determinations + 1),
                    limit: limits.determinations,
                    category: category.rawValue
                )
            case .logs:
                context.logs = try await logs.read(
                    interval: interval,
                    categories: [],
                    warningsOnly: configuration.warningsOnly,
                    maxBytes: limits.logBytes
                )
                context.notes
                    .append(
                        "Logs are bounded to newest matching timestamped lines (\(limits.logBytes) bytes). Continuation lines and older file tails may be omitted."
                    )
            }
        }
        return context
        ...
        -> [AIHistorySummary]
    {
        ...
        func bucket(_ key: Int) -> AIHistorySummary {
            summaries[key] ?? AIHistorySummary(
                start: max(interval.start, Date(timeIntervalSince1970: Double(key) * 3600)),
        ...
        while start < interval.end {
            ...
            func selected<T: AIDatedEntry>(_ rows: [T], cap: Int, category: String) -> [T] {
                if rows.count > cap {
                    notes
                        .append(
                            "\(category) summary fetch truncated for interval beginning \(start.ISO8601Format()); counts and sums are partial."
                        )
                }
                return Array(rows.prefix(cap))
                    .filter { $0.date >= start && ($0.date < end || (end == interval.end && $0.date == end)) }
            }
            if configuration.categories.contains(.glucose) {
                ...
                for (key, values) in Dictionary(grouping: rows, by: { hourKey($0.date) }) {
                    ...
                    let total = values
                        .reduce(0.0) { $0 + Double($1.glucoseMgDL) } + (old?.meanMgDL ?? 0) * Double(old?.count ?? 0)
                    value.glucose = .init(
                        count: count,
            ...
            if configuration.categories.contains(.pumpHistory) {
                ...
                for (key, values) in Dictionary(grouping: rows, by: { hourKey($0.date) }) {
                    ...
                    value.pump = .init(
                        eventCounts: counts,
                        recordedPumpBolusUnits: pump,
                        recordedExternalInsulinUnits: external,
                        unclassifiedInsulinUnits: unclassified
                    )
                    summaries[key] = value
                }
            }
            if configuration.categories.contains(.carbs) {
                ...
                for (key, values) in Dictionary(grouping: rows, by: { hourKey($0.date) }) {
                    ...
                    value.carbs = .init(
                        count: values.count + (old?.count ?? 0),
            ...
            if configuration.categories.contains(.determinations) {
                ...
                for (key, values) in Dictionary(grouping: rows, by: { hourKey($0.date) }) {
                    var value = bucket(key)
                    value.determinations = .init(
                        count: values.count + (value.determinations?.count ?? 0),
                        enactedCount: values.filter(\.enacted)
                            .count + (value.determinations?.enactedCount ?? 0)
                    )
                    summaries[key] = value
                }
            }
