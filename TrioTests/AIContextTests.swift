...
@Suite("AI context and logs") struct AIContextTests {
    @Test("Summary boundaries do not double count meals, FPU, external insulin or pump records") func summaryBoundaries() async throws {
    ...
    @Test("Summary fetch caps report incomplete coverage") func summaryTruncation() async throws {
        let now = Date(timeIntervalSince1970: 1_800_000_000)
        let entry = AIGlucose(
            date: now.addingTimeInterval(-60),
            glucoseMgDL: 100,
            smoothedGlucoseMgDL: nil,
            direction: nil,
            isManual: false
        )
    ...
    @Test("Seven-day summaries cover old and recent hours without sending raw glucose rows") func weekSummary() async throws {
        let now = Date(timeIntervalSince1970: 1_800_000_000)
        let start = now.addingTimeInterval(-168 * 3600)
        let entries = (0 ..< 168).map { hour in
            AIGlucose(
                date: start.addingTimeInterval(Double(hour) * 3600 + 60),
                glucoseMgDL: 80 + hour,
                smoothedGlucoseMgDL: nil,
                direction: nil,
                isManual: false
        ...
        let builder = TrioAIContextBuilder(
            source: ContextSourceFixture(now: now, glucoseEntries: entries),
            logs: EmptyAILogs(),
            now: { now }
        )
        var configuration = AIConfiguration()
        ...
        #expect(try JSONEncoder().encode(context).count < 100_000)
    }

    @Test("An older requested interval excludes current readings") func historicalInterval() async throws {
        let now = Date(timeIntervalSince1970: 1_800_000_000)
    ...
    @Test("History is a moving selection, not the start of locally stored records") func rollingHistory() async throws {
        let firstDate = Date(timeIntervalSince1970: 1_800_000_000)
        var currentDate = firstDate
        let builder = TrioAIContextBuilder(
            source: ContextSourceFixture(now: firstDate),
            logs: EmptyAILogs(),
            now: { currentDate }
        )
    ...
    @Test(
        "Therapy selection includes settings and both glucose and insulin history without enabling sharing"
    ...
    @Test("Time window excludes old and future entries and retains newest values") func interval() async throws {
        ...
        #expect(context.intervalStart == Date(timeIntervalSince1970: 1_799_996_400))
    ...
    @Test("Rotated logs respect cutoff, category, severity and UTF8 size") func logs() async throws {
        ...
        let entries = try await reader.read(
            interval: DateInterval(
                start: formatter.date(from: "2026-09-11T00:00:00Z")!,
                end: formatter.date(from: "2026-09-11T05:00:00Z")!
            ),
        ...
        let limited = try await reader.read(
...
struct EmptyAILogs: AILogReader {
    func read(
        ...
        warningsOnly _: Bool,
        maxBytes _: Int
    ) async throws -> [AILogEntry] { [] }
}

struct FailingAISource: AITrioDataReading {
    func settings() async throws -> AISettingsSnapshot { throw AIError.invalidResponse }
    func glucose(in _: DateInterval, limit _: Int) async throws -> [AIGlucose] { throw AIError.invalidResponse }
    func pumpHistory(in _: DateInterval, limit _: Int) async throws -> [AIPumpEvent] { throw AIError.invalidResponse }
    func carbs(in _: DateInterval, limit _: Int) async throws -> [AICarbEntry] { throw AIError.invalidResponse }
    func determinations(in _: DateInterval, limit _: Int) async throws -> [AIDetermination] { throw AIError.invalidResponse }
}
...
struct ContextSourceFixture: AITrioDataReading {
    ...
    func glucose(in _: DateInterval, limit _: Int) async throws -> [AIGlucose] {
        if let glucoseEntries { return glucoseEntries }
        return [(-7200.0, 80), (-3600, 100), (-60, 130), (0, 140), (60, 200)].map {
            AIGlucose(
                date: now.addingTimeInterval($0.0),
                glucoseMgDL: $0.1,
                smoothedGlucoseMgDL: nil,
                direction: nil,
                isManual: false
    ...
    func pumpHistory(in _: DateInterval, limit _: Int) async throws -> [AIPumpEvent] { pumpEntries }
    func carbs(in _: DateInterval, limit _: Int) async throws -> [AICarbEntry] { carbEntries }
    func determinations(in _: DateInterval, limit _: Int) async throws -> [AIDetermination] { [] }
}

extension FailingAISource {
    func pumpBoundaryEvents(before _: Date) async throws -> [AIPumpEvent] { throw AIError.invalidResponse }
    func totalDailyDose(in _: DateInterval) async throws -> AITotalDailyDose? { throw AIError.invalidResponse }
    func adjustments(in _: DateInterval, limit _: Int) async throws -> [AIAdjustment] { throw AIError.invalidResponse }
}

extension ContextSourceFixture {
    func pumpBoundaryEvents(before _: Date) async throws -> [AIPumpEvent] { [] }
    func totalDailyDose(in _: DateInterval) async throws -> AITotalDailyDose? { nil }
    func adjustments(in _: DateInterval, limit _: Int) async throws -> [AIAdjustment] { [] }
}
