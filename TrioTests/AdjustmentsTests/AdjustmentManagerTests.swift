...
@Suite("Adjustment Manager Tests", .serialized) struct AdjustmentManagerTests: Injectable {
    ...
    @Test("Manual temp targets retire Sport across all command sources", arguments: [
        AdjustmentSource.app, .remote, .shortcut, .watch
    ]) func testTempTargetRetiresSport(source: AdjustmentSource) async throws {
        let presetID = await makeOverride(
            name: "Sport preset", enabled: false, date: Date(), duration: 45, indefinite: false
        )
        let rule = try await testContext.perform {
            let preset = try #require(self.testContext.existingObject(with: presetID) as? OverrideStored)
            return SportOverrideRule(activity: .running, overridePresetID: try #require(preset.id))
        }
        var settings = TrioSettings()
        settings.automaticSportEnabled = true
        settings.sportOverrideRules = [rule]
        let coordinator = SportModeCoordinator(settings: { settings }, makeContext: coreDataStack.newTaskContext)
        _ = try await coordinator.start(ruleID: rule.id)
        let targetID = await makeTempTarget(name: "Manual target", enabled: false, date: Date(), duration: 30, target: 140)

        try await manager.activateTempTarget(.objectID(targetID), source: source)

        let read = coreDataStack.newTaskContext()
        try await read.perform {
            let sport = try #require(read.fetch(OverrideStored.fetchRequest()).first { $0.sportRuleID == rule.id })
            #expect(!sport.enabled, "The manual target must end the automatic Sport override")
            #expect(!sport.isUploadedToNS)
            let runs = try read.fetch(OverrideRunStored.fetchRequest())
            #expect(runs.count == 1, "Sport is recorded once in history")
            #expect(runs.first?.override?.sportRuleID == rule.id)
            #expect(runs.first?.isUploadedToNS == false)
            let target = try #require(read.existingObject(with: targetID) as? TempTargetStored)
            #expect(target.enabled)
        }

        try await manager.cancelTempTarget(source: source)
        #expect(
            try await coordinator.start(ruleID: rule.id) == .previouslyHandled,
            "A duplicate workout must not restart Sport after manual replacement"
        )
    }
