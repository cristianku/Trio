...
Local verification on September 11, 2026 passed 65 tests covering rules, preset filtering, persistence, activation, conflicts, expiry, concurrency, save failure, SQLite migration and selected regressions. The Italian UI was verified on iPhone 17 Pro / iOS 26.2: creating a saved preset, opening Automatic Sport, selecting it for two activities, saving the links, opening the automation guide and retaining both links after relaunch. Physical Apple Watch delivery remains unvalidated: Shortcuts does not provide a verified original workout timestamp/UUID to this implementation. It cannot reliably distinguish a delayed first invocation from a current workout. A received action is not proof of a currently running workout. Before distribution or use for unattended therapy, verify start delivery, locking, disconnection/reconnection, queued events and the exact workout filters on the intended devices. If obsolete starts can be delivered without detectable provenance, use confirmation or revisit the automatic trigger design.
